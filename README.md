...
devices discover each other on a radar-style scanner, connect, and transfer
files peer-to-peer over a local socket connection.
...
A dark, neon-green themed interface (`#0F0F0F` background, `#39FF14` accent)
built around a custom animated radar view — nearby devices appear as pulsing
dots as the sweep passes over them, and tapping a revealed device connects
to it directly.
...
**2. Radar UI (`RadarView`)**
A custom `View` that renders the scanning animation — a rotating sweep arc,
range rings, and device "blips" that get revealed (with a pulse-ring
animation) as the sweep passes their position. Tapping a revealed device
triggers a connection attempt, same as picking it from the list view.

**3. Role negotiation (`RoleHandshake`)**
...
**4. File transfer (`FileSenderService`, `FileReceiverService`)**
...
- Custom `View` (Canvas-drawn) for the radar animation — no animation
  library dependency
